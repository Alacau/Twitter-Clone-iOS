...
//  TwitterClone
//
//  Created by Alan Cao on 4/24/20.
...
extension ProfileController: ProfileHeaderDelegate {
    ...
    func handleEditProfileFollow(_ header: ProfileHeader) {
        ...
        if user.isFollowed {
            ...
            UserService.shared.followUser(uid: user.uid) { (reference, error) in
                self.user.isFollowed = true
                self.collectionView.reloadData()
                
                NotificationsService.shared.uploadNotification(toUser: self.user, type: .follow)
            }
        }
    }
